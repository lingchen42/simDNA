...
class RNN:
    def __init__(self,
        ...
        if model_struct:
            ...
            if regularizers:
                self.model.add(Conv1D(input_shape=input_shape, 
                                      filters=nb_filters[0],
                                      kernel_size=kernel_size[0],
                                      padding='valid',
                                      activation=activation_method[0],
                                      strides=1,
                                      kernel_regularizer=w_regularizers[0],
                                      bias_regularizer=b_regularizers[0]))
            else:
                self.model.add(Conv1D(input_shape=input_shape, 
                                      filters=nb_filters[0],
                                      kernel_size=kernel_size[0],
                                      ...
                                      strides=1))
            if pool_size[0]:
                self.model.add(MaxPooling1D(pool_size=pool_size[0]))

            if regularization:
                self.model.add(TimeDistributed(Dropout(dropout_frac[0])))
            
            self.model.add(Bidirectional(LSTM(units=nb_filters[1], 
                                              return_sequences=True)))
    
            if regularization:
                self.model.add(Dropout(dropout_frac[1]))
